...
def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ---------------------------------
    # Parameters as in your Pine script (ADJUSTED for MEXC noise resilience)
    # Changed from rsi_len=6 to rsi_len=14 (less responsive to noise)
    # Changed from rsi_smooth=5 to rsi_smooth=10 (more smoothing)
    rsi_len_p = 14
    rsi_smooth_p = 10
    qqe_fact_p = 3.0
    ...
    if DEBUG_SIGNAL_LOGGING:
        ...
        for i in range(len(df)):
            final_signal = "NONE"
            if _log_bool(valid_short.iloc[i]):
                final_signal = "SHORT"
            elif _log_bool(valid_long.iloc[i]):
                final_signal = "LONG"

            payload = {
                "ts": _log_value(ts_series.iloc[i]),
                "close": _log_value(close.iloc[i]),
                "hull": _log_value(hull.iloc[i]),
                "hull_shift2": _log_value(hull_shift2.iloc[i]),
                "pQQE": _log_value(pQQE.iloc[i]),
                "sQQE": _log_value(sQQE.iloc[i]),
                "pRSI": _log_value(pRSI.iloc[i]),
                "sRSI": _log_value(sRSI.iloc[i]),
                "bb_up": _log_value(bb_up.iloc[i]),
                "bb_dn": _log_value(bb_dn.iloc[i]),
                "vol_osc": _log_value(osc.iloc[i]),
                "atr": _log_value(atr.iloc[i]),
                "atr_med": _log_value(atr_med.iloc[i]),
                "vol_z": _log_value(vol_z.iloc[i]),
                "vwap": _log_value(vwap_sess.iloc[i]),
                "cond_qqe_up": _log_bool(qqe_up.iloc[i]),
                "cond_qqe_dn": _log_bool(qqe_dn.iloc[i]),
                "cond_qqe_up_1st": _log_bool(qqe_up_1st.iloc[i]),
                "cond_qqe_dn_1st": _log_bool(qqe_dn_1st.iloc[i]),
                "cond_hull_up": _log_bool(hull_up.iloc[i]),
                "cond_hull_dn": _log_bool(hull_dn.iloc[i]),
                "cond_vol_ok": _log_bool(vol_ok.iloc[i]),
                "cond_vol_gate": _log_bool(vol_gate.iloc[i]),
                "cond_bias_long": _log_bool(bias_long.iloc[i]),
                "cond_bias_short": _log_bool(bias_short.iloc[i]),
                "long_signal_raw": _log_bool(long_signal_raw.iloc[i]),
                "short_signal_raw": _log_bool(short_signal_raw.iloc[i]),
                "valid_long": _log_bool(valid_long.iloc[i]),
                "valid_short": _log_bool(valid_short.iloc[i]),
                "final_signal": final_signal,
            }

            LOG.info("SIGNAL_EVAL %s", json.dumps(payload, sort_keys=True))
